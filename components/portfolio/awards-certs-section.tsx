...
export function AwardsCertsSection() {
  return (
    <section id="awards-certs" className="scroll-mt-24 border-t border-[var(--hairline)] py-12 lg:py-16">
            ...
            {portfolioData.awards.map((award, index) => (
              ...
              >
                <div className="flex items-baseline justify-between gap-4">
                  <p className="font-medium text-slate-900 dark:text-slate-100">{award.name}</p>
                  {award.date && (
                    <p className="shrink-0 font-mono text-xs tabular-nums text-slate-500">{award.date}</p>
                  )}
                </div>
